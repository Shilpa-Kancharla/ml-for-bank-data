...
yhat.bag <- predict(bank.bag, newdata = modified_test)
...
```{r}
set.seed(1)
bank.bag1 <- randomForest(y ~ ., data=modified_train, mtry = sqrt(19), ntree=500)
bank.bag1
bank.bag2 <- randomForest(y ~ ., data=modified_train, mtry = 19/3, ntree= 500)
bank.bag2
...
```{r}
yhat.bag1 <- predict(bank.bag1, newdata = modified_test)
mean((as.numeric(yhat.bag1) - as.numeric(modified_test$y))^2)

yhat.bag2 <- predict(bank.bag2, newdata = modified_test)
